...
router.get('/lighthouse', async (req, res, next) => {
    try {
        const { url } = req.query;
        const report = await performanceService.runPerformanceProfile(url);
...
router.get('/cpu', async (req, res, next) => {
    try {
        ...
        const profile = await performanceService.runCPUProfile(parseInt(duration));
        res.json({ success: true, data: profile });
    }
    catch (error) {
        next(error);
...
router.get('/network', async (req, res, next) => {
    try {
        const { url } = req.query;
        const analysis = await performanceService.analyzeNetworkRequests(url);
        res.json({ success: true, data: analysis });
    }
    catch (error) {
        next(error);
