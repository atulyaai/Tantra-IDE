...
export async function getFileTree(relativePath) {
    ...
    try {
        ...
        const validNodes = resolvedNodes.filter((node) => node !== null);
...
export async function searchFiles(pattern, relativePath = '.') {
    ...
    try {
        ...
        try {
            const { stdout } = await execAsync(`rg "${escapedPattern}" --json --max-count 1000 "${fullPath}"`, { maxBuffer: 10 * 1024 * 1024, timeout: 30000 });
            ...
            // Fallback to grep
            const { stdout } = await execAsync(`grep -rn --max-count 1000 "${escapedPattern}" "${fullPath}"`, { maxBuffer: 10 * 1024 * 1024, timeout: 30000 });
            return stdout.split('\n')
                .filter(line => line.trim())
                .map(line => {
...
export async function getFileStats(relativePath) {
    if (!isValidPath(relativePath)) {
        throw new Error('Invalid path: potential directory traversal detected');
    }
    const fullPath = path.join(WORKSPACE_PATH, relativePath);
