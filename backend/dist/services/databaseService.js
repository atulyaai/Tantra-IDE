...
export async function detectDatabaseConnections() {
    const connections = [];
    try {
...
function parseDatabaseConfig(content, filename) {
    const connections = [];
...
function detectDatabaseDependencies(packageJson) {
    const connections = [];
...
async function testMongoDBConnection(connection) {
    try {
...
async function executePostgreSQLQuery(connection, query) {
    ...
    try {
        ...
        const rows = lines.slice(2, -2).map(line => line.split('|').map(cell => cell.trim()));
        return {
            columns,
            rows,
            rowCount: rows.length,
...
async function executeSQLiteQuery(connection, query) {
    const startTime = Date.now();
    try {
        const dbPath = connection.database || connection.connectionString;
        if (!dbPath)
...
async function executeMongoDBQuery(connection, query) {
    const startTime = Date.now();
    try {
...
async function executeRedisQuery(connection, query) {
    const startTime = Date.now();
    try {
...
async function getPostgreSQLSchema(connection) {
...
async function getSQLiteSchema(connection) {
...
export function buildSelectQuery(table, columns = ['*'], where) {
    ...
    if (where && Object.keys(where).length > 0) {
        const conditions = Object.entries(where).map(([key, value]) => `${key} = ${typeof value === 'string' ? `'${value}'` : value}`);
...
export function buildUpdateQuery(table, data, where) {
    const sets = Object.entries(data).map(([key, value]) => `${key} = ${typeof value === 'string' ? `'${value}'` : value}`);
    const conditions = Object.entries(where).map(([key, value]) => `${key} = ${typeof value === 'string' ? `'${value}'` : value}`);
