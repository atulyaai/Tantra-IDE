...
async function scanDirectory(dirPath, mediaFiles) {
    try {
...
async function generateThumbnail(imagePath) {
    try {
        const thumbnailPath = path.join(path.dirname(imagePath), '.thumbnails', path.basename(imagePath, path.extname(imagePath)) + '_thumb.jpg');
...
export async function tagImageWithAI(imagePath) {
    try {
...
export async function findAssetUsage(assetPath) {
    const usageFiles = [];
    try {
...
async function searchInDirectory(dirPath, patterns, results) {
    try {
        const entries = await fs.readdir(dirPath, { withFileTypes: true });
        for (const entry of entries) {
            const fullPath = path.join(dirPath, entry.name);
...
export async function optimizeImage(imagePath, options) {
    try {
        ...
        const optimizedPath = path.join(path.dirname(fullPath), path.basename(fullPath, path.extname(fullPath)) + '_optimized.' + (options.format || 'jpg'));
...
export async function detectUnusedAssets() {
    const mediaFiles = await getAllMediaFiles();
    const unusedFiles = [];
    for (const file of mediaFiles) {
        const usage = await findAssetUsage(file.path);
